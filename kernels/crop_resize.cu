    ...
    int edge) 
{
    ...
    //rgbrgbrgb to rrrgggbbb +  MMM  // Planer format
    int area = dst_width * dst_height;
    float* pdst_c0 = size + dst + dy * dst_width + dx;
    ...
    cudaStream_t stream) 
{
    ...
    crop_resize_kernel<<<blocks, threads, 0, stream>>>(
        src + src_width*3*cropY + cropX*3, src_width*3, cropWidth,
        cropHeight, dst, dst_width,
        dst_height, 114, d2s, size, is_norm, jobs);
}