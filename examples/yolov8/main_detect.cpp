...
void postprocess(std::vector<Object>& objs, float* host_ptrs[1], PreParam pparam, int index, int num_channels, int num_anchors)
{
    ...
    // NMSBoxesBatched(bboxes, scores, labels, score_thres, iou_thres, indices);
...
std::shared_ptr<spdlog::logger> logger = spdlog::stdout_color_mt("yolov9");
...
int main(int argc, char *argv[]){
    ...
    SrcNetwork detector_v8(g_logger, format, min_NCHW, opt_NCHW, max_NCHW, INPUT_BLOB_NAME, OUTPUT_BLOB_NAME);
    ...
    if (!trt_file.good()) {
        detector_v8.buildEngine(strdup(onnx_path.c_str()), strdup(trt_path.c_str()));
        detector_context = detector_v8.getEngineContext(strdup(trt_path.c_str()), stream);
    }else{
        detector_context = detector_v8.getEngineContext(strdup(trt_path.c_str()), stream);
    }

    float* backbone_buffers[detector_v8.num_inputs + detector_v8.num_outputs];
    float* output_buffers[BatchSize * detector_v8.num_outputs];

    CUDA_CHECK(cudaMalloc((void**)&backbone_buffers[0], max_NCHW_[0] * max_NCHW_[1] * max_NCHW_[2] * max_NCHW_[3]  * sizeof(float)));    
    int i = 0;
    for (auto& bindings : detector_v8.output_bindings){
    ...
    for (size_t i = 0; i < file_names.size(); i += BatchSize) {
        ...
        //COPY RESULTS
        for (int i = 0; i < detector_v8.output_bindings.size(); i++)
        {
            size_t osize = detector_v8.output_bindings[i].size * detector_v8.output_bindings[i].dsize;
        ...
        int num_channels = detector_v8.output_bindings[0].dims.d[1];
        int num_anchors  = detector_v8.output_bindings[0].dims.d[2];
