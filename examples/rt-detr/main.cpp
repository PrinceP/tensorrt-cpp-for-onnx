...
void postprocess(std::vector<Object>& objs, float* host_ptrs[3], PreParam pparam, int index) {
    objs.clear();
    const int num_dets = 300; // Assuming number of detections per image is 300

    auto* scores = reinterpret_cast<float*>(host_ptrs[0]);
    auto* labels = reinterpret_cast<int*>(host_ptrs[1]);
    auto* boxes = reinterpret_cast<float*>(host_ptrs[2]);
    ...
    for (int i = 0; i < num_dets; ++i) {
        if(scores[index * num_dets + i] < 0.6){
            continue;
        }

        float* box_ptr = boxes + (index * num_dets + i) * 4;
        ...
        obj.prob = scores[index * num_dets + i];
        obj.label = static_cast<int>(labels[index * num_dets + i]);
        objs.push_back(obj);
    }
}
...
std::shared_ptr<spdlog::logger> logger = spdlog::stdout_color_mt("rt-detr");
...
int main(int argc, char *argv[]){
    ...
    char* format = "NCHW";
    vector<const char*> INPUT_BLOB_NAME =  {"images", "orig_target_sizes"};
    vector<const char*> OUTPUT_BLOB_NAME = {"scores", "labels", "boxes"};

    SrcNetwork detector_rtdetr(g_logger, format, min_NCHW, opt_NCHW, max_NCHW, INPUT_BLOB_NAME, OUTPUT_BLOB_NAME);
    ...
    if (!trt_file.good()) {
        detector_rtdetr.buildEngine(strdup(onnx_path.c_str()), strdup(trt_path.c_str()));
        detector_context = detector_rtdetr.getEngineContext(strdup(trt_path.c_str()), stream);
    }else{
        detector_context = detector_rtdetr.getEngineContext(strdup(trt_path.c_str()), stream);
    }

    float* backbone_buffers[detector_rtdetr.num_inputs + detector_rtdetr.num_outputs];
    float* output_buffers[BatchSize * detector_rtdetr.num_outputs];

    CUDA_CHECK(cudaMalloc((void**)&backbone_buffers[0], max_NCHW_[0] * max_NCHW_[1] * max_NCHW_[2] * max_NCHW_[3]  * sizeof(float)));    
    CUDA_CHECK(cudaMalloc((void**)&backbone_buffers[1], max_NCHW_[0] * 2  * sizeof(int)));    
    
    int i = 0;
    for (auto& bindings : detector_rtdetr.output_bindings){
        
        size_t size = bindings.size * bindings.dsize;
        
        CUDA_CHECK(cudaMalloc((void**)&backbone_buffers[2 + i], max_NCHW_[0] * size  *  sizeof(float)));
    ...
    for (size_t i = 0; i < file_names.size(); i += BatchSize) {
        ...
        int host_values_orig_target_sizes[max_NCHW_[0] * 2];
        for(int i = 0; i < max_NCHW_[0] * 2; i++)
            host_values_orig_target_sizes[i] = 0;
        ...
        for(int index=0; index < currentBatchSize; index++){
            ...
            host_values_orig_target_sizes[2*index + 0] = width;
            host_values_orig_target_sizes[2*index + 1] = height;
        ...
        //orig_target_sizes added 
        CUDA_CHECK(cudaMemcpyAsync(backbone_buffers[1], host_values_orig_target_sizes, max_NCHW_[0] * 2 * sizeof(int) , cudaMemcpyHostToDevice));
        ...
        //COPY RESULTS
        for (int i = 0; i < detector_rtdetr.output_bindings.size(); i++)
        {
            size_t osize = detector_rtdetr.output_bindings[i].size * detector_rtdetr.output_bindings[i].dsize;
            CUDA_CHECK(cudaMemcpyAsync(output_buffers[i],
                backbone_buffers[i + 2],
                osize,
        ...
        for(int index=0; index < currentBatchSize; index++){
            ...
            // Save image
            cv::imwrite("/app/results/rt-detr_" + img_name_batch[index], res);
