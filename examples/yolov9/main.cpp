...
int main(int argc, char *argv[]){
    ...
    if (!trt_file.good()) {
        detector_v9.buildEngine(strdup(onnx_path.c_str()), strdup(trt_path.c_str()));
        detector_context = detector_v9.getEngineContext(strdup(trt_path.c_str()), stream);
    }else{
        detector_context = detector_v9.getEngineContext(strdup(trt_path.c_str()), stream);
    }
    ...
    for (size_t i = 0; i < file_names.size(); i += BatchSize) {
        ...
        //COPY RESULTS
        for (int i = 0; i < detector_v9.output_bindings.size(); i++)
        {
            size_t osize = detector_v9.output_bindings[i].size * detector_v9.output_bindings[i].dsize;
        ...
        for(int index=0; index < currentBatchSize; index++){
            ...
            // Save image
            cv::imwrite("/app/results/v9_" + img_name_batch[index], res);
